    ...
    : QMainWindow(parent), _nextBlockSize(0)
{
    ...
    if (_settings->value("StatusBar").toString() != "")
        _shStatusBar = _settings->value("StatusBar").toBool();
    ...
    setWindowTitle("ServerIP: " + _ip +"    ServerPort: "+ QString::number(_port)+"    Nickname: "+_username+"    Status: "+_status);
    _widget = new QWidget;
    ...
    _lb = new QLabel("<H1>CLient<H1>");
    l->addWidget(_lb, 1, 1,1, 2, Qt::AlignLeft);
    l->addWidget(_info, 2 , 1, 1, 4);
    l->addWidget(_history, 4, 1, 1, 6);
    _history->setVisible(false);
    _history->setIconSize(QSize(320, 240));
...
void Client::create()
{
    ...
    statusBarr->addAction(yourStatus);
    settingsMenu->addMenu(statusBarr);
    ...
    QAction* shStatusBar = new QAction("Show statusbar");
    shStatusBar->setCheckable(true);
    connect(shStatusBar, &QAction::toggled, [=](bool i){
        if (i)
        {
            statusBar()->show();
            _shStatusBar = true;
        ...
        _settings->setValue("StatusBar", _shStatusBar);
    });
    shStatusBar->setChecked(_shStatusBar);
    settingsMenu->addAction(shStatusBar);
    //----------------------------------------------------------------- Справка
    ...
    QAction* history = new QAction("History");
    connect(history, SIGNAL(triggered()), SLOT(history()));
        ...
        for (auto j: QString("date ip name text").split(" "))
        {
            ...
            else if (j == "text")
            {
                ...
                else
                {
                    ...
                    QString text = _info->item(i)->text().remove(0, _info->item(i)->text().lastIndexOf(":") + 2);
    ...
    for (;;)
    {
        ...
        case comUserFile:
        {
            ...
            else
            {
                ...
                QLabel* label = new QLabel("Принять файл?");
...
void Client::showContextMenu(const QPoint &pos)
{
    ...
    myMenu.addAction("Send file", this, [=](){
        ...
        connect(view, &QPushButton::clicked, [=](){
            QFileDialog* fdialog = new QFileDialog(dialog, tr("Open file..."));
            fdialog->setAcceptMode(QFileDialog::AcceptOpen);
            fdialog->setFileMode(QFileDialog::ExistingFile);
            if(fdialog->exec() != QDialog::Accepted)
                return;
        ...
        connect(send, &QPushButton::clicked, [=](){
            ...
            QDataStream out(&block, QIODevice::WriteOnly);
            out << (quint64)0;
            out << (quint8)Client::comConcreteUserFile;
            QFile file = viewl->text();
            if (!file.open(QIODevice::ReadOnly))
            {
                QMessageBox::warning(this, "Warning","Can't open the file");
                return;
            }
            out << file.fileName().remove(0, file.fileName().lastIndexOf("/") + 1) + "," + _users->itemAt(pos)->text();
            QByteArray q = file.readAll();
            block.append(q);
            file.close();
            out.device()->seek(0);
...
void Client::updateFileProgress(qint64 bytes)
{
...
void makeReaderShift(QXmlStreamReader* reader, int shiftLength) {
...
void Client::history()
{
    if (isconnect)
    {
        QMessageBox::warning(this, "Warning","You need to leave first(pls :3)");
        return;
    }
    ...
    connect(open, &QAction::triggered, [=](){
        //        QDesktopServices::openUrl (QUrl("https://www.google.com/search?q=%D1%87%D1%82%D0%BE+%D0%BE%D1%82%D0%BA%D1%80%D1%8B%D1%82%D1%8C+%D0%B1%D0%BB%D1%8F%D1%82%D1%8C%3F+%D0%A7%D0%B5%D1%82%D0%BA%D0%BE+%D0%BF%D0%B8%D1%88%D0%B8%D1%82%D0%B5+%D0%B2+%D1%82%D0%B7+%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5+%D1%8F+%D0%BD%D0%B5+%D1%83%D0%BC%D0%B5%D1%8E+%D1%87%D0%B8%D1%82%D0%B0%D1%82%D1%8C+%D0%BC%D1%8B%D1%81%D0%BB%D0%B8&ei=JneSY8OqOs74qwH0n6xA&ved=0ahUKEwjD1fiUmuv7AhVO_CoKHfQPCwgQ4dUDCA8&uact=5&oq=%D1%87%D1%82%D0%BE+%D0%BE%D1%82%D0%BA%D1%80%D1%8B%D1%82%D1%8C+%D0%B1%D0%BB%D1%8F%D1%82%D1%8C%3F+%D0%A7%D0%B5%D1%82%D0%BA%D0%BE+%D0%BF%D0%B8%D1%88%D0%B8%D1%82%D0%B5+%D0%B2+%D1%82%D0%B7+%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5+%D1%8F+%D0%BD%D0%B5+%D1%83%D0%BC%D0%B5%D1%8E+%D1%87%D0%B8%D1%82%D0%B0%D1%82%D1%8C+%D0%BC%D1%8B%D1%81%D0%BB%D0%B8&gs_lcp=Cgxnd3Mtd2l6LXNlcnAQAzoKCAAQRxDWBBCwA0oECEEYAEoECEYYAFCJBVjaggFgkIACaARwAXgAgAFniAHlIZIBBDUwLjOYAQCgAQHIAQjAAQE&sclient=gws-wiz-serp") );
        QFileDialog* fdialog = new QFileDialog(this, tr("Open xml..."));
        fdialog->setAcceptMode(QFileDialog::AcceptOpen);
        fdialog->setFileMode(QFileDialog::ExistingFile);
        fdialog->setMimeTypeFilters(QStringList("(*.xml)"));
        if(fdialog->exec() != QDialog::Accepted)
            return;
        QFile file(fdialog->selectedFiles().constFirst());
        if (!file.open(QFile::ReadOnly | QFile::Text))
        {
            ...
            do {
                ...
                if (sr->name() == "message") {
                    ...
                    makeReaderShift(sr, 3);
                    qDebug() << messageTime << senderIP << senderName;
                    if (sr->name() == "text") {
                        makeReaderShift(sr, 1);
                        messageText = sr->text().toString();
                        _history->addItem(messageTime + ":" + senderIP + ":" + senderName + ":" + messageText);
                    } else if (sr->name() == "photo") {
                        //photo
                        makeReaderShift(sr, 1);
                        messageText = sr->text().toString();
                        sendedImage.loadFromData(QByteArray::fromBase64(messageText.toUtf8()));
                        QListWidgetItem* item = new QListWidgetItem(QIcon(sendedImage.scaled(320, 240)), "");
                        _history->addItem(messageTime + ":" + senderIP + ":" + senderName );
                        _history->addItem(item);
                    } else if (sr->name() == "file") {
                        //                        file
                        foreach(const QXmlStreamAttribute &attr, sr->attributes()) {
                            if (attr.name().toString() == "filename") {
                                QString attribute_value = attr.value().toString();
                                _history->addItem(messageTime + ":" + senderIP + ":" + senderName+ ":"+attribute_value);
                                makeReaderShift(sr, 1);
        ...
        fileMenu->addAction(exit);
        QMenu* view = menuBar()->addMenu("View");
        QAction* chat = new QAction("Chat");
        connect(chat, &QAction::triggered, [=](){
            menuBar()->clear();
            _lb->setVisible(true);
